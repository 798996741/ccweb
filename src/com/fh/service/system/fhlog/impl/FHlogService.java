...
public class FHlogService implements FHlogManager{
	 ...
	 * @throws Exception
	 */
	public void saveLog(String name,String ip,String cont,String type,String funtype,String fun,String filename,String fileurl,String logintype)throws Exception{

		PageData pd = new PageData();
		pd.put("name", name);					//用户名
		pd.put("ip", ip);						//ip
		pd.put("cont", cont);		//内容
		pd.put("type", type);//0、登录 1、功能操作
		pd.put("funtype", funtype);//功能类型
		pd.put("fun", fun);	//功能
		pd.put("filename", filename);		//内容
		pd.put("fileurl", fileurl);	//功能
		pd.put("logintype", logintype);		//0、后端登录1、前端登录
		dao.save("FHlogMapper.saveLog", pd);
	}
