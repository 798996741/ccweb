...
public class RoleController extends BaseController {
	...
	public ModelAndView list()throws Exception{
		...
		try{
			...
			String ROLE_NAME = pd.getString("ROLE_NAME")==null?"":URLDecoder.decode(URLDecoder.decode( pd.getString("ROLE_NAME"),"utf-8"),"utf-8");				//关键词检索条件
			if(null != ROLE_NAME && !"".equals(ROLE_NAME)){
				pd.put("ROLE_NAME", ROLE_NAME.trim());
			}
			PageData fpd = new PageData();
			fpd.put("ROLE_ID", "0");
			pd.put("parentId", "51b7bf40dad54bf6b8cc945ed226209f");
			List<PageData> roletype=dictionariesService.listAllDictByParentId(pd);
			mv.addObject("roletypeList", roletype);
			...
			pd.put("keywords", keywords.trim());//取得点击的角色组(横排的)
			mv.addObject("pd", pd);
		...
		try{
			...
			if(roleList_z.size() > 0){
				...
				if(userlist.size() > 0){						//此角色已被使用就不能删除
					...
					pd.put("ROLE_ID", ROLE_ID);
					PageData pd_s  = roleService.findObjectById(pd);
					
					if(pd_s.getString("ROLE_NAME").equals("工单专员")||pd_s.getString("ROLE_NAME").equals("部门领导")||pd_s.getString("ROLE_NAME").equals("投诉坐席")||pd_s.getString("ROLE_NAME").equals("部门处理人员")){
						errInfo = "false3";
					}else{
