...
public class FHlogController extends BaseController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/operatelog_list")
	public ModelAndView operatelog_list(Page page) throws Exception{
		//logBefore(logger, Jurisdiction.getUsername()+"列表FHlog");
		//if(!Jurisdiction.buttonJurisdiction(menuUrl, "cha")){return null;} //校验权限(无权查看时页面会有提示,如果不注释掉这句代码就无法进入列表页面,所以根据情况是否加入本句代码)
		ModelAndView mv = this.getModelAndView();
		...
		String mappername = pd.getString("mappername");				//关键词检索条件
		if(null != mappername && !"".equals(mappername)){
			pd.put("mappername",URLDecoder.decode( mappername.trim(), "UTF-8"));
		}
		String modulename = pd.getString("modulename");				//关键词检索条件
		if(null != modulename && !"".equals(modulename)){
			pd.put("modulename",URLDecoder.decode( modulename.trim(), "UTF-8"));
		}
		String lastStart = pd.getString("lastStart");	//开始时间
		String lastEnd = pd.getString("lastEnd");		//结束时间
		if(lastStart != null && !"".equals(lastStart)){
			pd.put("starttime", lastStart+" 00:00:00");
		}
		if(lastEnd != null && !"".equals(lastEnd)){
			pd.put("endtime", lastEnd+" 23:59:59");
		}
		pd.put("dwbm", user.getDWBM()); 
		page.setPd(pd);
		
		List<PageData> list =operatelogService.modulelistGroupByModulename(pd);
		List<PageData> varList = operatelogService.list(page);		//列出FHlog列表
		mv.setViewName("system/fhlog/operatelog_list");
		mv.addObject("varList", varList);
