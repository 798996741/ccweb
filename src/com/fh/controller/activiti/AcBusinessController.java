...
public class AcBusinessController extends BaseController {
	...
	protected void completeMyPersonalTask_Map(String taskId,Map<String, Object> vMap){
           ...
           // ProcessDiagramGenerator processDiagramGenerator = processEngine.getProcessEngineConfiguration().getProcessDiagramGenerator();	
            ...
            try {
                ...
                List<HistoricActivityInstance> historicActivityInstances = historyService
                        .createHistoricActivityInstanceQuery().processInstanceId(PROC_INST_ID_)
                        .orderByHistoricActivityInstanceStartTime().asc().list();
                for(HistoricActivityInstance historicActivityInstance : historicActivityInstances){
                    highLightedFinishes.add(historicActivityInstance.getActivityId());
                }
                imageStream = new DefaultProcessDiagramGenerator().generateDiagram(bpmnModel, 
                		"png", highLightedFinishes, 
                ...
                ProcessDiagramGenerator processDiagramGenerator = processEngine.getProcessEngineConfiguration().getProcessDiagramGenerator();	
                imageStream = processDiagramGenerator.generateDiagram(bpmnModel, "png", executedActivityIdList, flowIds, "宋体", "微软雅黑", "黑体", null, 2.0);	//使用默认配置获得流程图表生成器，并生成追踪图片字符流
	...
	private InputStream getResourceDiagramInputStream(String PROC_INST_ID_){
        try {
           ...
           //Flow_Get_Img_C flow_Get_Img_C=new Flow_Get_Img_C();
           //InputStream imageStream=flow_Get_Img_C.test(PROC_INST_ID_);
            
            
            HistoricProcessInstance processInstance = historyService.createHistoricProcessInstanceQuery().processInstanceId(PROC_INST_ID_).singleResult();
    		//获取流程图
    		...
    		List<HistoricActivityInstance> highLightedActivitList = historyService.createHistoricActivityInstanceQuery().processInstanceId(PROC_INST_ID_).list();
    		//高亮环节id集合
    		...
    		//当前流程实例执行到哪个节点
    		ExecutionEntity execution = (ExecutionEntity) runtimeService.createExecutionQuery().executionId(PROC_INST_ID_).singleResult();// 执行实例
    		if(execution!=null){
           ...
           /*Command<InputStream> cmd = new HistoryProcessInstanceDiagramCmd(  
        		   PROC_INST_ID_);  
           CommandContext c=Context.getCommandContext();
           System.out.println(c+"ddddd");
           InputStream imageStream =cmd.execute(c);*/
