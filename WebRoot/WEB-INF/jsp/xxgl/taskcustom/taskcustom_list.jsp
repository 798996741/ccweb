...
<body class="no-skin">
	<section class="content container-fluid">
		...
		<div class="seat-middle-top">
			...
			<form action="<%=path%>/taskcustom/list.do" id="Form_s" method="get">
		...
		<div class="seat-middle">
			<div class="seat-middle-row">
				<c:forEach items="${varList}" var="var" varStatus="vs">
					<div class="seat-middle-one" onclick="getCustom('${var.TEMPLATE_ID}','${var.TABLENAME}','${var.STATE}','${var.CUSTOM_TEMPLATE_ID}','${var.FPTYPE}','${var.ID}');">
						<div class="seat-middle-one-top">
							<div class="seat-middle-one-top-l">
								<canvas id="myCanvas${var.ID}" width="150" height="150"></canvas>
								<script type="application/javascript">
									var ids='${var.ID}';
									var zxqk='${var.NUMFP}';
									var num='${var.ALLNUM}';
									var sy=parseInt(zxqk)-parseInt(num);
									var bl=0;
									var color="";
									//alert(zxqk);
									//alert(num);
									if(zxqk!='0'){
										bl=parseFloat(parseInt(zxqk)*100/parseInt(num)).toFixed(2);
										setEchartsPie('myCanvas${var.ID}', [
		                                    {value:(num-bl), name:'未执行任务'},
		                                    {value:bl, name:'已执行任务'}
										]);
										color="#8ccc38";
									}else{
									...
									function setEchartsPie(id, data) {
									    ...
									    let option = {
								            ...
								            color:['#dddddd', '#8ccc38','#f7921e','blueviolet']
								    	};
							...
							<div class="seat-middle-one-top-m">
								<h3>${var.TASKNAME}</h3>
								<p>任务类型：${var.TYPENAME}</p>
								<p>完成期限：${var.COMPLETEDATE}</p>
								<p>分配模式：
								...
								<p>配额数：${var.ZXQK} 总记录:${var.ALLNUM}</p>
							</div>
							<div class="seat-middle-one-top-r">
								
								<c:if test="${QX.edit == 1 }">
									<div class="seat-t1">
										<a href="javascript:void(0)" onclick="imp('${var.CUSTOM_TEMPLATE_ID}','${var.TABLENAME}');">导入</a>
									</div>
									<div class="seat-t2">
										<a href="javascript:void(0)" onclick="toExcel('${var.CUSTOM_TEMPLATE_ID}');">导出</a>
									</div>
									<c:if test="${var.STATE == '0' }">
										<div class="seat-t3">
											<a href="javascript:void(0)" onclick="qidong('1','${var.ID}','${var.TEMPLATE_ID}','${var.TABLENAME}','${var.FPTYPE}');">执行</a>
										</div>
									</c:if>
									<c:if test="${var.STATE == '1' }">
										<div class="seat-t3">
											<a href="javascript:void(0)" onclick="qidong('2','${var.ID}','${var.TEMPLATE_ID}','${var.TABLENAME}');">结束</a>
										</div>
									</c:if>
									<c:if test="${var.STATE != '2' }">
										<div class="seat-t4">
											<a href="javascript:void(0)" onclick="choice_zx('${var.ID}','${var.TEMPLATE_ID}','${var.TABLENAME}','${var.FPTYPE}');">分配</a>
										</div>
									</c:if>
								</c:if>
						...
						<div <c:if test="${var.STATE==0||var.STATE==2}">class="seat-middle-one-bottom"</c:if>  <c:if test="${var.STATE==1}">class="seat-middle-one-bottoma"</c:if>>
...
<input type="hidden" value="" id="TABLENAME" name="TABLENAME">
	...
	<script type="text/javascript">
		...
		function tosearch(){
			...
			location.href="<%=path%>/taskcustom/list.do?keywords="+encodeURI(encodeURI(keywords));
			//$("#Form_s").submit();
		}
